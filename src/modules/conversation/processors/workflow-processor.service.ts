...
export class WorkflowProcessorService {
  ...
  constructor(private readonly eventBusService: EventBusService, private readonly workflowInstanceService: WorkflowInstanceService) { }
  ...
  async conversationStarted(workflowInstanceId: string, participant: string, value: string) {
    ...
    await this.eventBusService.emit('CONVERSATION_STARTED', {},  {workflowInstanceId, value, participant});
  }
  ...
  async conversationStopped(workflowInstanceId: string, participant: string,attribute: string, value: string, payload: Record<string, string>) {
    ...
    await this.eventBusService.emit( 'CONVERSATION_STOPPED', payload , { workflowInstanceId, stepId: attribute, value, participant });
  }
  ...
  async answerValid(workflowInstanceId: string, participant: string, attribute: string, value: string, payload: Record<string, string>) {
    ...
    await this.eventBusService.emit( 'ANSWER_VALID', payload, { workflowInstanceId, stepId: attribute, value, participant });
  }
  ...
  async answerInValid(workflowInstanceId: string, participant: string, attribute: string, value: string, payload: Record<string, string>) {
    ...
    await this.eventBusService.emit( 'ANSWER_INVALID', payload, { workflowInstanceId, stepId: attribute, value, participant });
  }
  ...
  async conversationCompleted(workflowInstanceId: string, participant: string, attribute: string, value: string, payload: Record<string, string>) {
    ...
    await this.eventBusService.emit( 'CONVERSATION_ENDED', payload, { workflowInstanceId, stepId: attribute, value, participant });
  }

  async createWorkFlow(workflowId: string, conversationId: string, trigger: string, attribute: string): Promise<string> {
